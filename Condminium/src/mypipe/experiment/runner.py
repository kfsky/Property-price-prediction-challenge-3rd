...
class Runner:
    ...
    def __init__(self, config, run_params, model_params, fit_params, model, features, use_mlflow=False):
        ...
        self.metrics = run_params["metrics"]
        self.seeds = run_params["seeds"] if run_params["seeds"] is not None else [71]
    ...
    # モデルの学習を行う関数（foldごとに行う関数）
    def train_fold(self, seed, i_fold):
        """
        :param seed: seed
        :param i_fold: set n_fold (default=5)
    ...
    # 学習を行う関数（foldの結果をまとめ、oof・scoreまで算出する）
    # 算出後は、結果を保存する
    ...
    # 予測モデルでの関数
    # モデルを読み込んで、seedごとに予測する。予測結果を平均化し保存
    def run_predict_cv(self) -> None:
        ...
        self.logger.info(f'{self.exp_run_name} =======  Complete prediction cv 🎉 ======= ')
        self.preds = preds

    # foldごとにtrain_dataのidxを取得する関数
    ...
    def load_x_train(self):
        ...
        num = num if num is not None else(cols)
        cols = cols[:num]
